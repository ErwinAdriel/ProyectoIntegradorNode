# AvComputing 🚀
Proyecto final para curso Back-End Node Js. Este proyecto esta divido en 2 secciones, por un lado backend, para la logica, modelos, vista, controladores. Y por otro lado frontend, basado en Vite y React para las views. 
...
* Express Node Js: Framework web minimo y flexible, crea servidores web API, y aplicaciones.
...
* Node js: entorno de ejecución de código abierto y multiplataforma que permite ejecutar código JavaScript fuera del navegador, específicamente en el lado del servidor.
* Express js: framework web minimalista y flexible para Node.js, escrito en JavaScript, que facilita enormemente la creación de aplicaciones web y APIs robustas y escalables del lado del servidor.
* React Router DOM: Es una librería de JavaScript para React que facilita la creación de enrutamiento en aplicaciones web.
* Firebase, firestore: base de datos NoSQL flexible y escalable de Google, parte de la plataforma Firebase, diseñada para almacenar, sincronizar y consultar datos de aplicaciones móviles y web a escala global.
...
```bash
git clone https://github.com/ErwinAdriel/ProyectoIntegradorNode.git
```
2. BackEnd, instala las dependencias
```bash
cd backend
```
```bash
npm install
```
3. FrontEnd, instala las dependencias
```bash
cd ..
```
```bash
...
npm install
```
3. Iniciá el servidor de desarrollo BackEnd:
```bash
npm run start o npm run dev
```
4. Iniciá el servidor de desarrollo FrontEnd:
```bash
npm run dev
```
## 🌐 Deploy 
...
- FrontEnd: https://frontend-avcomputing.vercel.app
